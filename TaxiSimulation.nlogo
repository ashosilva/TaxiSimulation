...
to setup
  ...
  create-turtles num-taxis [
    ...
    set shape "car"
    set color white
    set heading (one-of [0 90 180 270])  ;; Set random heading: 0 (right), 90 (up), 180 (left), 270 (down)
  ]
...
to generate-rides
  if random 1 = 0 [
    ...
    if (pickup-spot != nobody and dropoff-spot != nobody) [
      ...
      ask pickup-spot [set pcolor green]  ;; Mark pickup location
      ask dropoff-spot [set pcolor red]  ;; Mark dropoff location
    ]
  ]
end
...
to dispatch-taxis [strategy]
  foreach ride-requests [ request ->  ;; Define 'request' explicitly
    let pickup-location first request
    let chosen-taxi nobody

    if strategy = "nearest" [
      set chosen-taxi min-one-of turtles with [not has-passenger?] [distancexy (item 0 pickup-location) (item 1 pickup-location)]
    ]
    if strategy = "smart" [
      ; involving traffic
    ]
...
to move-taxis
  ask turtles [
    ;when dispatched it goes to the pickup location
    if dispatched? [

      let pickup-location first destination
      let pickup-patch patch (first pickup-location) (item 1 pickup-location)

      move-algo pickup-patch ;calls move

      ;; Pick up passenger if at pickup location
      if distance pickup-patch < 1 [
        set color blue
    ...
    ;when passenger is picked up it goes to the dropoff location
    if has-passenger?[

      let dropoff-location item 1 destination
      let dropoff-patch patch (first dropoff-location) (item 1 dropoff-location)

      move-algo dropoff-patch ;calls move

      ;; Drop off passenger if at dropoff location
      if distance dropoff-patch < 1 [
        set has-passenger? false
        set color white
...
;;Movement function (moving towars target patch)
to move-algo [tpatch]
  if tpatch = nobody [ stop ]  ;; Safety check

  ;; Get the best neighboring patch based on street availability and traffic
...
to go
  generate-rides
  dispatch-taxis dispatch-strategy  ;; Change strategy here
  move-taxis
  ;update-taxi-plots
  tick
end
